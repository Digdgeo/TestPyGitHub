...
def split_basemaps(m, layers_dict=None, left_name=None, right_name=None, width="120px", **kwargs):
...
def PhenoApp(m=None):
    ...
    #Adding eLETR shapes
    #NEED TO CHANGE SHAPEFILES PATH TO GEE TABLES DONE!
    eelter_object = ee.FeatureCollection('projects/ee-digdgeografo/assets/elter_lyon')
    eeDonana_object = eelter_object.filter(ee.Filter.eq('layer', 'Donana'))
    eeBaixo_object = eelter_object.filter(ee.Filter.eq('layer', 'BaixoSabor'))
    eeriverexe_object = eelter_object.filter(ee.Filter.eq('layer', 'RiverExe'))
    eeCairngorms_object = eelter_object.filter(ee.Filter.eq('layer', 'Cairngorms'))
    eeVeluwe_object = eelter_object.filter(ee.Filter.eq('layer', 'Veluwe'))
    eeGranparadiso_object = eelter_object.filter(ee.Filter.eq('layer', 'Gran_Paradiso'))
    eeSchorfheide = eelter_object.filter(ee.Filter.eq('layer', 'Schorfheide'))
    #eeMinho_object = eelter_object.filter(ee.Filter.eq('name', 'LTSER Minho Estuary'))
    eeNeusiedler_object = eelter_object.filter(ee.Filter.eq('layer', 'Neusiedler'))
    #eeAtelier_object = eelter_object.filter(ee.Filter.eq('name', 'LTSER Zone Atelier Alpes'))

    eLTER_SITES = {'Baixo': 
                    ['Baixo Sabor LTER - Portugal', eeBaixo_object],
                    'Donana': 
                    ['Donana Long-Term Socio-ecological Research Platform - Spain', eeDonana_object],
                    'River_Exe': 
                    ['River Exe - United Kingdom', eeriverexe_object],
                    'Cairngorms': 
                    ['Cairngorms National Park LTSER - United Kingdom', eeCairngorms_object],
                    'Veluwe': 
                    ['LTSER Veluwe - Netherlands', eeVeluwe_object],
                    'Gran_Paradiso': 
                    ['Gran Paradiso National Park - Italy', eeGranparadiso_object],
                    'Schorfheide': 
                    ['Schorfheide-Chorin - Germany - Germany', eeSchorfheide],
                    'Neusiedler':
                    ['LTSER Platform Neusiedler See - Seewinkel - Austria', eeNeusiedler_object]
                    }
    ...
    # 'LTSER Zone Atelier Armorique - France',
    ...
    site = widgets.Dropdown(
        options=[i for i in eLTER_SITES.keys()],
        #     "Donana",
        #     "Craighorn",
        #     "Grand Paradiso",
        # ],
    ...
    ndvi2gif = widgets.Checkbox(
        value=True,
        description="Show Ndvi2Gif composite",
        tooltip="Show last season NDVI composite",
        style=style,
    )
    ...
    float_Xtext = widgets.FloatText(
        ...
        step=0.001,
        description='Latitude:',
        layout=widgets.Layout(width="160px", padding="0px 0px 0px 0px")
        )
    ...
    float_Ytext = widgets.FloatText(
        ...
        step=0.001,
        description='Longitude:',
        layout=widgets.Layout(width="165px", padding="0px 0px 0px 0px")
        )
    ...
    def dropdown_change(change):
        if change['new']:
            ...
            if ndvi2gif.value==True:
                ...
                MyClass = NdviSeasonality(geom, 2018, 2020, 'Sentinel', 'perc_90')
    ...
    def legend_change(c):   
        ...
        if legend.value == True and phenometrics_val.value == 'Doy':
            ...
            m.add_legend(legend_keys=legend_keys, legend_colors=legend_colors, legend_title="Month", position='bottomleft')   
            ...
            m.add_legend(legend_keys=legend_keys, legend_colors=legend_colors, legend_title="NDVI", position='bottomleft')
    ...
    def submit_clicked(b):
        ...
        if m is not None and collection.value == "MODIS MCD12Q2.006":
            ...
            m.addLayer(vegetationrs, modis_vis, 'MODIS')
            ...
            if phenometrics_val.value == 'Doy':
                ...
                m.addLayer(nbase, s2_vis_, 'SENTINEL 2 VPP Doy')
                ...
                m.addLayer(nbase, s2_vis, 'SENTINEL 2 VPP Value')

            
            #m.add_legend(legend_keys=legend_keys, legend_colors=modis_vis['palette'], position='bottomleft')       
    ...
    def close_click(change):
        with output:
            output.clear_output()
            print('Downloading raster to the current folder...')
        # if m is not None:
        #     m.toolbar_reset()
        #     if m.tool_control is not None and m.tool_control in m.controls:
        #         m.remove_control(m.tool_control)
    ...
    toolbar_footer.children = [
        ...
        widgets.HBox([title, bands]),
        #widgets.HBox([start_month, start_month_label, end_month, end_month_label]),
        #widgets.HBox([float_Xtext, float_Ytext]),
        ndvi2gif,
        legend,
        #nd_indices,
        #widgets.HBox([first_band, second_band]),
        #widgets.HBox([nd_threshold, nd_threshold_label, nd_color]),
        widgets.HBox([load_rasters, close_btn, reset_btn]),
        output,
    ]
...
def WaterDetect(m=None):
    ...
    #Adding eLETR shapes
    lter_shp = '/home/diego/EBD/eLTER/PhenoApp/shapes/tool/elter_full_phen_big.shp'
    eelter_object = shp_to_ee(lter_shp)

    donana_shp = '/home/diego/EBD/eLTER/PhenoApp/shapes/tool/Donana.shp'
    eedonana_object = shp_to_ee(donana_shp)

    baixo_shp = '/home/diego/EBD/eLTER/PhenoApp/shapes/tool/Baixo.shp'
    eebaixo_object = shp_to_ee(baixo_shp)

    eisenwurzen_shp = '/home/diego/EBD/eLTER/PhenoApp/shapes/tool/Eisenwurzen.shp'
    eeeisenwurzen_object = shp_to_ee(eisenwurzen_shp)

    Cairngorms_shp = '/home/diego/EBD/eLTER/PhenoApp/shapes/tool/Cairngorms.shp'
    eeCairngorms_object = shp_to_ee(Cairngorms_shp)

    Venice_shp = '/home/diego/EBD/eLTER/PhenoApp/shapes/tool/Venice.shp'
    eeVenice_object = shp_to_ee(Venice_shp)

    Armorique_shp = '/home/diego/EBD/eLTER/PhenoApp/shapes/tool/AtellierNancy.shp'
    eeArmorique_object = shp_to_ee(Armorique_shp)

    Rhine_shp = '/home/diego/EBD/eLTER/PhenoApp/shapes/tool/Rin.shp'
    eeRhine_object = shp_to_ee(Rhine_shp)


    eLTER_SITES = {'Baixo Sabor': 
                    ['Baixo Sabor LTER - Portugal', eebaixo_object],
                    'Donana Natural Space': 
                    ['Doñana Long-Term Socio-ecological Research Platform - Spain', eedonana_object],
                    'Eisenwurzen': 
                    ['LTSER Platform Eisenwurzen (EW) - Austria', eeeisenwurzen_object],
                    'Cairngorms': 
                    ['Cairngorms National Park LTSER - United Kingdom', eeCairngorms_object],
                    'Venice': 
                    ['IT16-Lagoon of Venice - Italy', eeVenice_object],
                    'Armorique': 
                    ['LTSER Zone Atelier Armorique - France', eeArmorique_object],
                    'Rhine': 
                    ['Rhine-Main-Observatory - Germany', eeRhine_object]
                    }
    ...
    def dropdown_change(change):
        if change['new']:
            ...
            if ndvi2gif.value==True:
                ...
                MyClass = NdviSeasonality(geom)
                median = MyClass.get_year_composite().median()
                vizParams = {'bands': ['spring', 'autumn', 'summer'], 'min': 0, 'max': 0.9}
                m.addLayer(median, vizParams, 'median')
    ...
    def submit_clicked(b):
        ...
        cur_date = datetime.date(start_year.value, 1, 1) #Today's date
        start_date = datetime.date(1970, 1, 1) #Jan 1 1970
        min_val = (today - past_date).days
        max_val = min_val + 365
        ...
        if m is not None:
            ...
            m.addLayer(vegetationrs, modis_vis, 'median')
